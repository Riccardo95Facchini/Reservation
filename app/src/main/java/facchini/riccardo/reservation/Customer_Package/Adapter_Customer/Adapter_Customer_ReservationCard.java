...
public class Adapter_Customer_ReservationCard extends RecyclerView.Adapter<Adapter_Customer_ReservationCard.Reservation_Customer_ViewHolder>
{
    ...
    public Adapter_Customer_ReservationCard(Context context, List<ReservationFirestore> reservationCustomerHomeList)
    {
    ...
    public void onBindViewHolder(@NonNull Reservation_Customer_ViewHolder holder, int pos)
    {
        ...
        holder.textName.setText(res.getShopName());
        holder.textAddress.setText(res.getWhere());
        holder.textWhen.setText(res.timeFormatted());
        Glide.with(context).load(res.getShopPic()).placeholder(R.drawable.default_avatar).fitCenter().centerCrop().transform(new CircleCrop()).into(holder.profilePic);
    }
