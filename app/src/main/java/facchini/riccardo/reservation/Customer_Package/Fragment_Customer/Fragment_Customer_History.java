...
public class Fragment_Customer_History extends Fragment implements OnItemClickListener, OnReservationListener
{
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState)
    {
        ...
        final CountDownTimer timer = new CountDownTimer(5000, 500)
        {
            ...
            {
                if (reservationViewModel.getPastReservations().getValue() == null || reservationViewModel.getPastReservations().getValue().isEmpty())
                    showReservations(null);
            }
        }.start();
        
        
        reservationViewModel.getPastReservations().observe(getActivity(), new Observer<List<ReservationFirestore>>()
        {
            @Override
        ...
        reservationViewModel.getIsPastEmpty().observe(getActivity(), new Observer<Boolean>()
        {
            @Override
    ...
    {
        final ReservationFirestore res = reservationViewModel.getPastReservations().getValue().get(position);
